...
const ProductList = forwardRef(({ isConnected, account }, ref) => {
  ...
  const handleBuyProduct = async (product) => {
    ...
    setBuyMessage('');
    try {
      // 1. Khởi tạo provider & signer
      const provider = new ethers.providers.Web3Provider(window.ethereum);
      const signer = provider.getSigner();

      // 2. Khởi tạo contract (giữ nguyên address & ABI của bạn)
        ...
        <div className="modal-overlay" onClick={handleCloseModal}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {/* Nút mua sản phẩm */}
              {account && selectedProduct.owner && selectedProduct.owner.toLowerCase() === selectedProduct.registeredBy.toLowerCase() && account.toLowerCase() !== selectedProduct.owner.toLowerCase() && (
